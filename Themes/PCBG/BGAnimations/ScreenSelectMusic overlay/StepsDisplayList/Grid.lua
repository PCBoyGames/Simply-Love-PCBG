...
local GridRows = 5
local GridZoomX = IsUsingWideScreen() and 0.295 or 0.26
local BlockZoomY = 0.275
local StepsToDisplay, SongOrCourse, StepsOrTrails

local t = Def.ActorFrame{
	Name="StepsDisplayList",
	InitCommand=cmd(vertalign, top; draworder, 1; xy, _screen.cx-170, _screen.cy + 78),
	-- - - - - - - - - - - - - -
...
local Grid = Def.ActorFrame{
	Name="Grid",
	InitCommand=cmd(horizalign, left; vertalign, top; xy, 10, -52 ),
}
...
for RowNumber=1,GridRows do
	...
	Grid[#Grid+1] = Def.BitmapText{
		...
		InitCommand=function(self)
			...
			self:y(RowNumber * height * BlockZoomY)
			self:x( IsUsingWideScreen() and -155 or -140 )
			self:zoom(0.3)
		end,
		SetCommand=function(self, params)
