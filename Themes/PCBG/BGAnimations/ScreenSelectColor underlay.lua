...
local wheel_item_mt = {
	__index = {
		create_actors = function(self, name)
			...
			local af = Def.ActorFrame{
				...
				InitCommand=function(subself)
					...
					subself:bounce():effectmagnitude(0,0,0):effectperiod(0.666):effectoffset(1)
				end,
				OffCommand=function(subself)
					subself:sleep(0.04*self.index+0.1)
					subself:decelerate(0.8)
					subself:y(-500)
		...
		transform = function(self, item_index, num_items, has_focus)
			...
			local X_SpaceBetweenHearts = IsUsingWideScreen() and ((_screen.w / (num_items-1))+25) or ((_screen.w / (num_items))+25)
			local OffsetFromCenter = (item_index - math.floor(num_items/2))-1
			local x = X_SpaceBetweenHearts * OffsetFromCenter
			local z = -2 * math.abs(OffsetFromCenter/5)
			...
			if IsUsingWideScreen() then
				local y = (4 * math.pow(OffsetFromCenter,2)) - 20
				self.container:rotationz( OffsetFromCenter * 5 )
