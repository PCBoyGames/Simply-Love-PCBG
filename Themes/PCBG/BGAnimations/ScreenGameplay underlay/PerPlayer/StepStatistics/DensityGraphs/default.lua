...
local InitializeNPSHistogram = function()
	...
	if HasData() then
		...
		local yellow = {0.05, 0.125, 0.225, 1}
		local orange = {0.1, 0.25, 0.45, 1}
		local upper
		...
		for i, nps in ipairs(NPSperMeasure) do
			...
			if i > 2 and verts[#verts][1][2] == y and verts[#verts-2][1][2] == y then
				...
				upper = lerp_color(math.abs(x), yellow, orange )

				verts[#verts+1] = {{x, y, 0}, yellow} -- bottom of graph (yellow)
				verts[#verts+1] = {{x, 0, 0}, upper}  -- top of graph (somewhere between yellow and orange)
			end
		end
	end
end
...
if HasData() then
	...
	local density_graph_and_lifeline = Def.ActorFrame{
		...
		SampleCommand=function(self)
			...
			if current_second > TotalSeconds - (max_seconds*WideScale(0.01,0.5)) then return end

			-- use 1/4 of whatever max_seconds is as the cutoff to start scrolling the graph
			local seconds_past_one_fourth = current_second-(max_seconds*0.01)

			if seconds_past_one_fourth > 0 then
				local offset = scale(seconds_past_one_fourth, 0, TotalSeconds-(max_seconds*0.01), 0, width-(GetNotefieldWidth(player)*0.01))
				self:x(-offset)
			end
		end,
