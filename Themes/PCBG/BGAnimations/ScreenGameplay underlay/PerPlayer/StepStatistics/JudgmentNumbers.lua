...
for index, window in ipairs(TapNoteScores) do
	...
	t[#t+1] = Def.BitmapText{
		...
		InitCommand=function(self)
			...
			leadingZeroAttr = { Length=3, Diffuse=Brightness(SL.JudgmentColors[SL.Global.GameMode][index], 0) }
		...
		JudgmentMessageCommand=function(self, params)
			...
			if params.TapNoteScore and ToEnumShortString(params.TapNoteScore) == window then
				...
				leadingZeroAttr = { Length=(4-tonumber(tostring(TapNoteJudgments[window]):len())), Diffuse=Brightness(SL.JudgmentColors[SL.Global.GameMode][index], 0) }
				self:AddAttribute(0, leadingZeroAttr )
			end
		end
	}
...
for index, RCType in ipairs(RadarCategories) do
	...
	t[#t+1] = Def.BitmapText{
		...
		BeginCommand=function(self)
			...
			leadingZeroAttr = { Length=2, Diffuse=color("#000000") }
			self:AddAttribute(0, leadingZeroAttr )
		end,
		JudgmentMessageCommand=function(self, params)
			if params.Player ~= player then return end
			...
			leadingZeroAttr = { Length=(3-tonumber(tostring(RadarCategoryJudgments[RCType]):len())), Diffuse=color("#000000") }
			self:AddAttribute(0, leadingZeroAttr )
		end
	}
